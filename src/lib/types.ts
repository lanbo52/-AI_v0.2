...
export interface Project {
  ...
  description?: string;
  previewImage?: string;
  isBackgroundLocked?: boolean; // [NEW] Validation lock
  createdAt: number;
  updatedAt: number;
}
...
export interface AgentMessage {
  ...
  content: string;
  timestamp: number;
  // [NEW] Structure for System Activity Logs
  activityLog?: {
    role: string;
    status: AgentStatus;
    task: string;
    logs: string[];
    validationFeedback?: string; // Detailed Aligner feedback
  };
  action?: {
    type: 'auto_fix';
    targetFile: string;
...
export interface EpisodeProgress {
  ...
  // [NEW] Sequential Workflow Fields
  isLocked?: boolean;     // 是否锁定 (前一集未完成)
  canAdvance?: boolean;   // 是否可以继续下一集 (当前已完成)
}
